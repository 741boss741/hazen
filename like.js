...
let LUser;
  ...
  let rows = sql.prepare(`SELECT * FROM profile WHERE UserID = '${pUser.id}'`).get()
    ...
    if(!rows) return message.reply(":x: | `غير مسجل بعد`").then(msg => {msg.delete(5000)});
    LUser  = rows.likes;
    ...
    if(rows.length < 1) return message.reply(":x: | `غير مسجل بعد`").then(msg => {msg.delete(5000)});
    let LUser  = rows.likes;
          ...
          if(allliked[i].LikedUser === pUser.id){
            ...
            return message.channel.send(`:x: | __${prettyMs((timeleft), {verbose: true})}__** من قبل <@${pUser.id}>  تم الإعجاب بـ **`).then(msg => {msg.delete(10000)});
    ...
    sqlstr = `INSERT INTO liked (UserID, LikedUser, GuildID, Time) VALUES ('${message.author.id}', '${pUser.id}', '${message.guild.id}', '${currentTime}')`
           sql.prepare(sqlstr).run()
              sqlstr = `UPDATE profile SET likes = ${LUser + 1} WHERE UserID = '${pUser.id}'`;
              sql.prepare(sqlstr).run()
              return message.reply("**:white_check_mark: | تم الإعجاب**").then(msg => {msg.delete(5000)});
