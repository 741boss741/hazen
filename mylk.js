...
module.exports.run = async (bot, message, args, sql) => {
    ...
    message.reply( ":white_check_mark: | `تم إرسال قائمة الفانزات بالخاص`").then(msg => {msg.delete(5000)});
        let names = '';
      let rowliked = sql.prepare(`SELECT * FROM liked WHERE LikedUser = '${message.author.id}'`).all()
        if(!rowliked) return message.reply(":x: | `للأسف لا توجد لديك لايكات`").then(msg => {msg.delete(5000)});
